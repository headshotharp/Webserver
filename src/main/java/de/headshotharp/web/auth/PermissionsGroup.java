...
public enum PermissionsGroup {
	BEGINNER("[B]", 0, Color.BLACK, ""), PLAYER("[P]", 1, new Color(0, 140, 50), "player"), TRUSTED_PLAYER("[T]", 2,
			new Color(0, 140, 50),
			"trusted"), DEVELOPMENT("[DEV]", 3, new Color(255, 140, 26), "development"), COMMUNITY_MANAGER("[CM]", 4,
					new Color(230, 220, 0), "cm"), ADMIN("[SA]", 5, new Color(253, 62, 255), "admin");
	...
	/**
	 * returns found {@link PermissionsGroup} or {@link PermissionsGroup.PLAYER} if
	 * not found
	 * 
	 * @param name
	 * @return
	...
	public static PermissionsGroup byBukkitName(String name) {
		for (PermissionsGroup g : values()) {
			if (g.bukkitname.equalsIgnoreCase(name))
				return g;
		}
		return PermissionsGroup.PLAYER;
	}
