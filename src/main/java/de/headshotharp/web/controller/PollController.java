...
public class PollController extends DefaultController {
		...
		if (poll != null) {
			if (cd.getAuthentication().isLoggedIn()) {
				...
				if (result > 0) {
					...
					tableResult.append(
							"<table class='table-sm table-bordered'><tr><th>Nr.</th><th>Option</th><th>Stimmen</th></tr>");
					int index = 0;
					int totalVotes = 0;
					...
					tableResult.append(
							"<tr><td></td><td><p>Gesamt</p></td><td class='text-right'>" + totalVotes + "</td></tr>");
					...
					cd.getModel().addAttribute("scripts", new String[] { svgDonutResult.toString() });
				} else {
					String html = "<h3>Umfrage:</h3><p><b>" + poll.title + "</b></p><p>" + poll.description + "</p>";
					for (PollOption option : poll.options) {
						html += "<p><form class='inline-block' method='post' action='/poll'><input type='hidden' name='pollId' value='"
								+ poll.id + "' /><input type='hidden' name='optionId' value='" + option.id
								+ "' /><input class='btn-sm btn-primary' type='submit' value='Auswählen' /></form> "
								+ option.polloption + "</p>";
					}
					cd.getModel().addAttribute("content", html);
				}
	...
	@PostMapping("/poll")
	String pollPost(@ModelAttribute("ControllerData") ControllerData cd, @RequestParam("pollId") int pollId,
