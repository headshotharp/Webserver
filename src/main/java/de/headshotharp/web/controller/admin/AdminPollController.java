...
public class AdminPollController extends DefaultController {
		...
		if (cd.getAuthentication().isLoggedIn()) {
			...
			if (player.hasPermission(permissionsGroup)) {
				cd.getModel().addAttribute("content",
						Bootstrap.button("Zurück", "/admin", ButtonType.DEFAULT)
	...
	@GetMapping("/add")
	String pollAdd(@ModelAttribute("ControllerData") ControllerData cd,
	...
	@PostMapping("/add")
	String pollAddPost(@ModelAttribute("ControllerData") ControllerData cd,
			@RequestParam Map<String, String> requestParams) {
		cd.getModel().addAttribute("bg", "");
		if (cd.getAuthentication().isLoggedIn()) {
			Player player = cd.getAuthentication().getPlayer();
			if (player.hasPermission(permissionsGroup)) {
				...
				if (title.length() == 0) {
					cd.getModel().addAttribute("content",
							"<p class='bg-danger'>Es muss ein Titel eingegeben werden.</p>" + makeForm(requestParams));
				} else {
					...
					if (options.size() >= 2) {
						...
						cd.getModel().addAttribute("content",
								"<p class='bg-danger'>Es müssen mindestens 2 Optionen ausgefüllt werden.</p>"
										+ makeForm(requestParams));
					}
				}
			} else {
				cd.getModel().addAttribute("content", "<p>Es ist dir nicht erlaubt den Blog zu bearbeiten.</p>");
	...
	private String makeForm(@RequestParam Map<String, String> requestParams) {
		...
		html += "<div class='form-group'><label>Titel:</label><input type='text' class='form-control' name='title' value='"
				+ title
				+ "'></div><div class='form-group'><label>Beschreibung:</label><textarea class='form-control' name='desc' rows='5'>"
				+ desc
