	...
	String shop(@ModelAttribute("ControllerData") ControllerData cd)
	{
		...
		if (cd.getAuthentication().isLoggedIn())
		{
			...
			if (player.hasPermission(permissionsGroup))
			{
				...
				cd.getModel().addAttribute("content", Bootstrap.button("Zurück", "/admin", ButtonType.DEFAULT) + "<br /><hr /><h3>Administration Shop</h3><form method='post' action='/admin/shop'><label>Rabatt (in %, 0 bis 30):</label><input type='text' class='form-control' name='discount' value='" + discount + "' /><br /><input class='btn btn-success' type='submit' value='Bestätigen' /></from>");
			}
			else
			{
			...
			if (player.hasPermission(permissionsGroup))
			{
				...
				try
				{
					...
					if (discount >= 0 && discount <= 30)
					{
						...
						html = "<p>Rabatt für Enchantments auf <b>" + db_discount + "%</b> gesetzt.</p>" + Bootstrap.button("Zurück", "/admin/shop", ButtonType.SUCCESS);
					}
					else
					{
						html = "<p>Deine Eingabe war ungültig.</p>" + Bootstrap.button("Zurück", "/admin/shop", ButtonType.SUCCESS);
					}
				}
