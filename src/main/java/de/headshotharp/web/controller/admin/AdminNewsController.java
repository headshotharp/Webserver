...
public class AdminNewsController extends DefaultController {
	...
	@PostMapping("/add")
	String newsAddPost(@ModelAttribute("ControllerData") ControllerData cd, @RequestParam("title") String title,
			@RequestParam("msg") String msg, @RequestParam("type") int type) {
		cd.getModel().addAttribute("bg", "");
		if (cd.getAuthentication().isLoggedIn()) {
			Player player = cd.getAuthentication().getPlayer();
			if (player.hasPermission(permissionsGroup)) {
		...
		if (cd.getAuthentication().isLoggedIn()) {
			...
			if (player.hasPermission(permissionsGroup)) {
				...
				String html = "<h3>Eintrag bearbeiten:</h3><form accept-charset='UTF-8' method='post' action='/admin/news/change'><input type='hidden' name='newsId' value='"
						+ news.id
						+ "' /><div class='form-group'><label>Titel:</label><input type='text' class='form-control' name='title' id='txt_title' value='"
						+ news.title
						+ "'></div><div class='form-group'><label>Nachricht:</label><textarea class='form-control' id='txt_msg' name='msg' rows='15'>"
						+ CommonUtils.br2nl(news.msg) + "</textarea></div>";
				html += "<div class='form-group'><label>Typ:</label><fieldset><input type='radio' name='type' value='0'"
						+ (type == 0 ? " checked='checked'" : "")
						+ " /> <label>Standard</label><br /><input type='radio' name='type' value='2'"
						+ (type == 2 ? " checked='checked'" : "")
						+ " /> <label>Positiv</label><br /><input type='radio' name='type' value='1'"
						+ (type == 1 ? " checked='checked'" : "") + " /> <label>Negativ</label><br /></fieldset></div>";
	...
	@PostMapping("/change")
	String newsChangePost(@ModelAttribute("ControllerData") ControllerData cd, @RequestParam("newsId") int newsId,
			@RequestParam("title") String title, @RequestParam("msg") String msg, @RequestParam("type") int type) {
		cd.getModel().addAttribute("bg", "");
		if (cd.getAuthentication().isLoggedIn()) {
			Player player = cd.getAuthentication().getPlayer();
			if (player.hasPermission(permissionsGroup)) {
