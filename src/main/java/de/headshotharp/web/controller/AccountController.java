...
public class AccountController extends DefaultController {
	...
	String account(@ModelAttribute("ControllerData") ControllerData cd,
		...
		if (cd.getAuthentication().isLoggedIn()) {
			...
			svg += "</div><div class='col-xs-4 text-center'><b>" + dt.toMonthAndYear()
					+ "</b></div><div class='col-xs-4 text-center'>";
			if (year != now.year || month != now.month) {
				DateTime next = dt.nextMonth();
				svg += " <a href='/account?year=" + next.year + "&month=" + next.month
						+ "#stats' class='btn btn-success pull-right'><span class='desktop-only'>Nächster Monat</span> <span class='glyphicon glyphicon-arrow-right'></span></a>";
			}
			svg += "</div></div><br />";
			...
			cd.getModel().addAttribute("scripts",
					new String[] { "$(document).ready(function(){updateUserProgressbars();});" });
			cd.getModel().addAttribute("template", "account");
		} else {
			cd.getModel().addAttribute("bg", "");
			cd.getModel().addAttribute("content",
					"<p>Du musst eingeloggt sein, um den Accountbereich zu betreten.</p>");
		}
		return "index";
	}
	...
	public String accountsettings(@ModelAttribute("ControllerData") ControllerData cd) {
		...
		if (cd.getAuthentication().isLoggedIn()) {
			...
			String html = "<h3>Einstellungen</h3><h4>Account: " + player.name
					+ (player.realname.length() > 0 ? " <small>(" + player.realname + ")</small>" : "") + "</h4>";
	...
	String changepw(@ModelAttribute("ControllerData") ControllerData cd, @RequestParam("oldpw") String oldpw,
			@RequestParam("newpw") String newpw, @RequestParam("newpw2") String newpw2) {
		cd.getModel().addAttribute("bg", "");
		if (cd.getAuthentication().isLoggedIn()) {
