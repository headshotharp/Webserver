...
public class MainController extends DefaultController
{
	...
	String bestlist(@ModelAttribute("ControllerData") ControllerData cd)
	{
		...
		{ jsUserId + Player.getJavascriptData(cd.getDataProvider().getPlayerListActive()) });
	...
	String gift(@ModelAttribute("ControllerData") ControllerData cd)
	{
		...
		if (cd.getAuthentication().isLoggedIn())
		{
			...
			if (player.giftReady)
			{
				...
				cd.getModel().addAttribute("content", "<p>Du hast " + giftAmount + " " + Config.VALUE_CURRENCY_HTML + " erhalten.</p>");
			}
			else
			{
				cd.getModel().addAttribute("content", "<p>Du hast dein Geschenk bereits abgeholt. Bitte komme morgen wieder.</p>");
			}
		}
		else
		{
			cd.getModel().addAttribute("content", "<p>Du musst eingeloggt sein, um dein tägliches Geschenk zu erhalten.</p>");
		}
		return "index";
	}
	...
	String chat(@ModelAttribute("ControllerData") ControllerData cd)
	{
		...
		{
			cd.getModel().addAttribute("bg", "");
			cd.getModel().addAttribute("content", "<p>Du musst dich <a href='/login'>einloggen</a> um den Chat zu sehen.</p>");
		}
		return "index";
	}
}
