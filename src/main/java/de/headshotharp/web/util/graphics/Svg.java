...
public class Svg
{
	public static String lineGraph(Timeline timeline)
	{
		...
		str.append(legendHighlight(sideoffset, legend_right_offset - sideoffset, legend_height_part, yoffsetlegend, legend.length, "#eee"));
		...
		for (TimelinePart tlp : timeline.getParts())
		{
			...
			// text
			str.append(part("text", true, "x", x + barwidth + 1, "y", baseline + fontSize * 3 / 2, "font-size", fontSize + "px", "text-anchor", "middle", tlp.timestamp.day + "."));
			i++;
		}
		str.append("</g>");
		str.append(endSvg());
	...
	public static String legendLeft(int width, int height_part, int line, int yoffset, int... values)
	{
		...
		for (int i = 0; i < values.length; i++)
		{
			...
			str.append(part("line", true, "x1", x1, "y1", y, "x2", width, "y2", y, "stroke", "black", "stroke-width", "1"));
			str.append(part("text", true, "x", x1 - 5, "y", y + fontSize / 2, "font-size", fontSize + "px", "text-anchor", "end", values[values.length - 1 - i]));
		}
		str.append(part("line", true, "x1", width, "y1", yoffset, "x2", width, "y2", height_part * (values.length - 1) + yoffset, "stroke", "black", "stroke-width", "1"));
		return str.toString();
	}
	...
	public static String legendRight(int xoffset, int height_part, int line, int yoffset, int... values)
	{
		...
		{
			y = height_part * i + yoffset;
			str.append(part("line", true, "x1", xoffset, "y1", y, "x2", xoffset + line, "y2", y, "stroke", "black", "stroke-width", "1"));
			str.append(part("text", true, "x", xoffset + line + 5, "y", y + fontSize / 2, "font-size", fontSize + "px", "text-anchor", "start", values[values.length - 1 - i]));
		}
		str.append(part("line", true, "x1", xoffset, "y1", yoffset, "x2", xoffset, "y2", height_part * (values.length - 1) + yoffset, "stroke", "black", "stroke-width", "1"));
		return str.toString();
	}

	public static String legendHighlight(int xoffset, int width, int height_part, int yoffset, int amount, String color)
	{
		StringBuilder str = new StringBuilder();
		int y;
	...
	{
		if (stroke.equalsIgnoreCase("same")) stroke = fill;
		if (fill.equalsIgnoreCase("same")) fill = stroke;
		return part("rect", true, "x", x, "y", y, "width", width, "height", height, "stroke", stroke, "fill", fill, "stroke-width", 1);
	}
